...
const isValidEmailOrPhone = (value) => {
  ...
  return EMAIL_REGEX.test(value) || PHONE_REGEX.test(value.replaceAll(" ", ""));
};
...
const LoginPage = () => {
  ...
  const emailOrPhoneIsValid = useMemo(
  ...
  const passwordIsValid = passwordStrength.score >= 2;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emailOrPhoneIsValid) {
      nextErrors.emailOrPhone = "Enter a valid email address or phone number.";
    }

    if (!passwordIsValid) {
      nextErrors.password = "Add 8+ characters with numbers and a symbol.";
    }
    ...
    try {
      ...
      const redirectPath = `/${data.demographic}-dashboard`;
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 px-4 py-16 sm:px-6 lg:px-10">
      ...
      <div className="pointer-events-none absolute inset-0 opacity-80">
        ...
        <div className="absolute -bottom-32 right-10 h-72 w-72 rounded-full bg-cyan-500/20 blur-3xl" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_-10%,rgba(59,130,246,0.2),transparent_40%)]" />
      </div>
        ...
        <div className="flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/10 shadow-[0_40px_120px_rgba(15,23,42,0.45)] backdrop-blur-2xl lg:flex-row">
          <aside className="flex flex-col gap-8 bg-gradient-to-br from-indigo-500/20 via-transparent to-transparent p-10 text-slate-100 lg:w-5/12">
            <div className="inline-flex items-center gap-3 rounded-full border border-white/20 bg-white/10 px-5 py-2 text-sm font-semibold uppercase tracking-wide">
              ...
              CyberSafe Enterprise Login
            </div>

            <div>
              <h2 className="text-3xl font-semibold tracking-tight">Stay in control of every alert.</h2>
              <p className="mt-3 text-sm leading-relaxed text-slate-200/80">
                Sign in to command personalized threat reports, automated guardrails, and faster escalation workflows your team trusts.
              </p>
            </div>

            <ul className="space-y-3 text-sm text-slate-200/80">
              <li className="flex items-start gap-3">
                <span className="mt-1 flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/30 text-emerald-300">
                  <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </span>
                Manage incident alerts and AI-driven recommendations in one cockpit.
              </li>
              <li className="flex items-start gap-3">
                <span className="mt-1 flex h-6 w-6 items-center justify-center rounded-full bg-sky-500/30 text-sky-300">
                  <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M4 7h16M4 12h10M4 17h7" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </span>
                Customize protection policies by region, device posture, and risk appetite.
              </li>
              <li className="flex items-start gap-3">
                ...
                Ship executive-grade reports with automated narratives and benchmarks.
              </li>
            </ul>
          </aside>
            ...
            <div className="mx-auto w-full max-w-md">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Enterprise-grade access with continuous encryption and adaptive policies.
                  </p>
                </div>
                <div className="hidden h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/10 shadow-inner shadow-white/10 sm:flex">
                  <svg className="h-7 w-7 text-indigo-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
                    <path d="M12 2l7 4v5c0 5-3 9-7 10-4-.8-7-5-7-10V6l7-4z" strokeLinecap="round" strokeLinejoin="round" />
              ...
              <form onSubmit={handleSubmit} className="mt-10 space-y-6" noValidate>
                  ...
                  <div>
                    ...
                    <div className="group relative flex items-center rounded-2xl border border-white/10 bg-white/5 transition duration-200 focus-within:border-indigo-400/70 focus-within:bg-white/10 focus-within:shadow-[0_0_30px_rgba(99,102,241,0.25)]">
                      ...
                      <input
                        ...
                        placeholder="name@company.com / +91 90000 00000"
                      />
                      {emailOrPhoneIsValid && formData.emailOrPhone && (
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <label className="flex items-center gap-3 text-sm text-slate-300">
                    <span className="relative inline-flex items-center">
                      <input
                        ...
                        className="peer h-4 w-4 cursor-pointer appearance-none rounded border border-slate-500/60 bg-transparent transition checked:border-indigo-400 checked:bg-indigo-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                      />
                  ...
                  <button
                    type="button"
                    className="flex w-full items-center justify-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-2 text-sm text-slate-200 transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 sm:w-auto sm:justify-start"
                  >
              ...
              <div className="mt-10 space-y-4 text-sm text-slate-300">
                <p className="flex items-center gap-2 text-xs uppercase tracking-[0.25em] text-slate-400">
                  <span className="h-[1px] w-12 bg-slate-700" /> Security brief
                </p>
                <p className="text-sm">
                  Your credentials are encrypted in transit and at rest. Our infrastructure is continuously monitored for SOC 2 compliance.
                </p>
